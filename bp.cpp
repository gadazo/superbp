...
typedef struct  {
  ...
  typedef enum  {SNT = 0, WNT, WT, ST} state;
//Global
  int btb_size;
  int history_size;
  int tag_size;
...
int BP_init(unsigned btbSize, unsigned historySize, unsigned tagSize,
    ...
    if (is_global_table){
      global_sm = new state [pow(2,history_size)];
      //TODO:intializing thr global state machine table
    ...
    // if not global history need to add history for each btb_line
          btb_table[i].history = new uint32_t;
          btb_table[i].history_size = 0;
        }
        if (is_global_table) {
            btb_table[i].state_mach = global_sm;
        } else{
          //if not global table need to add state table size of pow(2,history_size)
            btb_table[i].state_mach = new state [pow(2, history_size)];
...
int get_sm_index (uint32_t history , uint32_t pc){
  ...
  if (shared_type = 0 ){
    ...
    //lsb shared type
    pc_masked = pc/4;
    pc_masked = pc_masked & (pow(2,history_size)-1);
...
bool BP_predict(uint32_t pc, uint32_t *dst){
  ...
  if (btb_table[btb_index].is_valid){
    ...
    if (btb_table[btb_index].tag = ((pc/4) & (pow(2,tag_size)-1))){
...
void BP_update(uint32_t pc, uint32_t targetPc, bool taken, uint32_t pred_dst){
  ...
  bool tag_compare = (btb_table[btb_index].tag != ((pc/4) & (pow(2,tag_size)-1)));
  if ((btb_ent->is_valid) && tag_compare) || (!btb_ent->is_valid){
    btb_ent->tag = ((pc/4) & (pow(2,tag_size)-1));
    if (!is_global_hist){
      btb_ent->history =0;
    }
    if (!is_global_table){
  ...
  if (taken) {
    switch (*cur_state) {
      case SNT: *cur_state = WNT; break;
      case WNT: *cur_state = WT; break;
      case WT:
      case ST: *cur_state = ST; break;
    }
  } else {
    switch (*cur_state) {
      case ST: *cur_state = WT; break;
      case WT: *cur_state = WNT; break;
      case WNT:
      case SNT: *cur_state = SNT; break;
  }
  uint8_t prev_history = btb_table[btb_index].history;
  btb_table[btb_index].history = (prev_history*2) & (pow(2,history_size)-1);
